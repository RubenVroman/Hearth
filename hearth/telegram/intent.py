...
_SYSTEM = (
    ...
    "Also action=retry when a known library title has no file, is too big, or won't "
    "play well and they want another / smaller version "
    "('too big', 'won't play', 'download another one', 'te groot', 'andere versie'). "
    "Also action=retry when the title is already in the library and they want ANOTHER "
    "download without deleting the current file "
    "('already there but find another', 'don't delete the old one', 'download another "
    "copy', 'download a new version', 'keep the current file'). That is keep-both — "
    "not Overseerr re-request. "
    ...
    "bad queue release or offers alternate Radarr releases for a library title. "
    "If Radarr returns no_alternate / no grab-able releases, tell the user clearly — "
    "never escalate that to Chief of Staff. "

...
def _pretty_tool(name: str, data: dict[str, Any]) -> str | None:
    ...
    if name == "radarr_queue":
        ...
        if spoken:
            ...
            # speak already ends with punctuation; keep mock tag readable.
            if mock and spoken.endswith("."):
                return spoken[:-1] + " (mock)."
            return f"{spoken}{suffix}"
        if data.get("empty"):
            return f"Nothing active in the Radarr queue{mock}."
        return f"Radarr queue{mock}: {data.get('count') or 0} item(s)."
...
# Progress / queue status must win over _GRAB (which also matches “download”).

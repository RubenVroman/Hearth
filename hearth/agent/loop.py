    ...
    async def _run_local(self, user_text: str) -> dict[str, Any]:
        ...
        if plan is None:
            reply = (
                ...
                "Apple TV, grab movies in Radarr or shows in Sonarr, request via Overseerr, "
                "Plex now-playing, workspace, docker inspect. Repo, Gridways, Discord, calendar, "
                "and anything I can't do yet go to Chief of Staff."
            )
            runtime.note("assistant", reply)
...
def _format_tool_reply(tools: list[dict[str, Any]]) -> str:
    ...
    for tool in tools:
        ...
        if tool.get("needs_confirm"):
            ...
            spoken = _confirm_line(name, preview, plan=data.get("plan") if isinstance(data.get("plan"), dict) else None)
            parts.append(spoken)
            continue
        if not tool.get("ok"):
            data = tool.get("data") or {}
...
def _confirm_line(name: str, preview: dict[str, Any], plan: dict[str, Any] | None = None) -> str:
    if name == "chief_of_staff":

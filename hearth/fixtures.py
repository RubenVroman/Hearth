...
# Library titles for queue fallback (completed / missing, not actively downloading).
...
class MockPipeline:
    ...
    def find_radarr_library(self, query: str) -> list[dict[str, Any]]:
        needle = (query or "").strip().lower()
        if not needle:
            return []
        hits = [
            deepcopy(item)
            for item in MOCK_RADARR_LIBRARY
            if needle in str(item.get("title", "")).lower()
            or all(tok in str(item.get("title", "")).lower() for tok in needle.split() if tok)
        ]
        return hits

...
USER_ID = 42
_jpeg = io.BytesIO()
Image.new("RGB", (30, 30), "blue").save(_jpeg, format="JPEG")
JPEG = _jpeg.getvalue()
...
def test_batch_ceiling_fits_a_sixteen_poster_grid() -> None:
    ...
    assert Settings.model_fields["telegram_vision_mode"].default == "auto"
...
async def test_poster_becomes_a_get_card_and_does_not_queue(
    ...
    assert provider.calls and provider.calls[0]["nbytes"] > 0
...
@pytest.mark.asyncio
async def test_auto_mode_requests_confident_exact_title(
    vision_bot: Callable[..., tuple[TelegramMediaBot, QueryOverseerr, FixtureVisionProvider]],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert "Host" in reply.text
    assert reply.reply_markup is None
    assert len(overseerr.request_calls) == 1
...
async def test_openai_provider_failure_does_not_log_the_image(
    ...
    monkeypatch.setattr("hearth.telegram.media.image_requests.AsyncOpenAI", Boom)
...
def test_vision_rate_environment_aliases_share_one_setting() -> None:
    canonical = Settings.model_validate({"HEARTH_TELEGRAM_VISION_PER_MINUTE": 7})
    compatibility = Settings.model_validate({"HEARTH_TELEGRAM_VISION_RATE_PER_MINUTE": 9})
    assert canonical.telegram_vision_rate_per_minute == canonical.telegram_vision_per_minute == 7
...
@pytest.mark.asyncio
async def test_fallback_only_on_retryable_failure_and_always_closed(monkeypatch, failure):
    from hearth.telegram.media import vision_provider as providers
    monkeypatch.setattr(settings, "telegram_vision_fallback", "local")
    error = (
        providers.VisionSchemaError("invalid shape") if failure == "schema"
        else providers.VisionUnavailable("auth failed", retryable=False) if failure == "authentication"
        else providers.VisionUnavailable("timeout", retryable=True)
    )
    primary = providers.FixtureVisionProvider(error)
    fallback = providers.FixtureVisionProvider(VisionResult(kind="not_media"))
    built = []
    closed = []
    def build(name):
        built.append(name)
    ...
    async def close():
        closed.append(True)
    fallback.aclose = close
    monkeypatch.setattr(providers, "build_vision_provider", build)
    if failure == "timeout":
        result = await providers.identify_image(primary, JPEG, "image/jpeg", None)
        assert result.kind == "not_media"
        assert built == ["local"] and closed == [True]
    else:
        with pytest.raises(type(error)):
            await providers.identify_image(primary, JPEG, "image/jpeg", None)
        assert not built and not closed

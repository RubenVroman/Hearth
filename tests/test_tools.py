...
async def test_radarr_queue_library_completed_when_not_downloading():
    result = await registry.call("radarr_queue", {"query": "The Endless"})
    assert result.ok
    assert result.data["matched"] is True
    assert result.data["library"] is True
    assert result.data["items"][0]["status"] == "completed"
    assert "on disk" in result.data["speak"].lower() or "completed" in result.data["speak"].lower()


async def test_radarr_queue_library_missing_when_not_downloading():
    result = await registry.call("radarr_queue", {"query": "Brutalist"})
    assert result.ok
    assert result.data["matched"] is True
    assert result.data["items"][0]["status"] == "missing"
    assert "missing" in result.data["speak"].lower()


async def test_radarr_queue_not_in_radarr_clear_fallback():
    result = await registry.call("radarr_queue", {"query": "Totally Missing Film"})
    assert result.ok
    assert result.data["empty"] is True
    assert result.data["count"] == 0
    assert "not in radarr" in result.data["speak"].lower()

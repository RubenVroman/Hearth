...
5. VAULT's steady state is enforce (`HEARTH_JEV_ENABLED=true`, `HEARTH_JEV_SHADOW=false`). Use shadow (`HEARTH_JEV_SHADOW=true`) while tuning a new risky path: it logs `suggested` vs `action` and does not change allow/deny. Expand only after smoke tests in [docs/telegram-media-smoke.md](../telegram-media-smoke.md) and the ops steps in [docs/jev.md](../jev.md), then return VAULT to enforce.

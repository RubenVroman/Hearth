...
- Ambiguous titles get a top-3 disambiguation; reply `1` / `2` / `3`, `all of them`, `the first one`, or `the new one`. Franchise one-shots like `the whole Harry Potter series` queue the related set. Vague plot/character asks (e.g. “a movie about a boy with glasses who is a wizard”) resolve to a catalog title via the same cheap `OPENAI_MODEL` intent layer when configured, then take the normal search path. Intent uses `OPENAI_MODEL` (default `gpt-4o-mini`) when configured, with a local heuristic fallback. Exact catalog ids grab immediately.

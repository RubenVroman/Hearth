...
| `HA_CLIMATE_ENTITY` / `HA_PURIFIER_ENTITY` / `HA_FEEDER_ENTITY` | Pin one entity per role. Empty = discover by domain and name. Set these from what `ha_discover_entities` reports after pairing. |
| `HA_PET_FEEDER_ENTITIES` / `HA_AIRCO_ENTITIES` / `HA_AIR_PURIFIER_ENTITIES` | Search order used when nothing is pinned — commonly seen Tuya naming, not ids observed on this house. |
| `HA_PET_FEEDER_COOLDOWN_SECONDS` | Anti-double-feed window. Default `600`. A repeat inside it needs “feed them anyway”. |
| `HA_PET_FEEDER_MAX_PORTIONS` / `HA_PET_FEEDER_DEFAULT_PORTIONS` | Portions per feed and the cap. Defaults `6` / `1`. |
| `HA_AIRCO_DEFAULT_MODE` | Which hvac mode “airco on” picks when the unit offers several. Default `cool`. |
| `TUYA_LAN_HOSTS` | Tuya device addresses to health-check on TCP 6668 (`tuya_lan_probe`). Private addresses only; Hearth never sweeps a subnet. |
...
- Climate, pet feeder, and air purifier → `house_climate` / `house_feeder` / `house_purifier`, all via Home Assistant services (no Tuya client). Optional `HA_CLIMATE_ENTITY`, `HA_PURIFIER_ENTITY`, `HA_FEEDER_ENTITY`; empty means discover by domain and name. `house_comfort` and `GET /api/comfort` feed the dashboard chips. Telegram offers a one-tap reply keyboard for those devices when HA actually has them. Jev gates all three inside the tool registry, so chat, voice, Telegram and `/api/invoke` share one decision.
- Which Home Assistant entity *is* the feeder / airco / purifier → `ha_discover_entities` or `GET /api/devices/discover`. Filters by domain and keyword, reports per-role readiness and ready-to-paste `.env` lines, and — because Home Assistant only loads a custom integration once it has a config entry — tells you when `tuya_local` is installed but has adopted nothing, which looks identical to "not paired" from entities alone. Read-only. Pairing walkthrough: [docs/devices.md](docs/devices.md).
- Is the Tuya hardware even on the network → `tuya_lan_probe` (TCP 6668, hosts from `TUYA_LAN_HOSTS`, private addresses only). Separates “not paired in HA” from “unplugged”; discovery runs it automatically when a device is missing. An open port proves a Tuya device is at that address, never which one — Hearth will not name a device from a port scan.
- Those devices speak English and Dutch: “feed the cats”, “geef de katten eten”, “airco 21”, “zet de airco uit”, “purifier 40%”, “luchtreiniger op auto”. Food cannot be un-dispensed, so a repeat feed inside `HA_PET_FEEDER_COOLDOWN_SECONDS` needs “feed them anyway”.
...
- Vision is not rate-limited by default. `HEARTH_TELEGRAM_VISION_PER_MINUTE` and `HEARTH_TELEGRAM_VISION_DAILY_CAP` are `0` (unlimited); a positive value is an optional cap. One chat still waits with `I'm still reading your previous image` while a previous image is in flight. Images are not retained or sent to Jev; only extracted media coordinates enter its decision scope. Disabled vision, `shadow` mode, or a missing provider key leave attachments unprocessed.
- Progress checks Radarr/Sonarr only for titles this bot queued.

The relevant tuning variables are `TELEGRAM_RATE_LIMIT_PER_MINUTE`, `TELEGRAM_MAX_TITLE_LENGTH`, `TELEGRAM_PROGRESS_INTERVAL_SECONDS`, `TELEGRAM_CONCURRENCY`, `TELEGRAM_CALLBACK_TTL_SECONDS`, `TELEGRAM_DB_PATH`, the lane switches (`HEARTH_TELEGRAM_MOOD_LANE`, `HEARTH_TELEGRAM_PERSON_LANE`, `HEARTH_TELEGRAM_SIMILAR_LANE`, `HEARTH_TELEGRAM_BATCH_LANE`, `HEARTH_TELEGRAM_BATCH_MAX_ITEMS` default 4 / ceiling 16, `HEARTH_TELEGRAM_CONTEXT_TTL_SECONDS`, `HEARTH_TELEGRAM_BUTLER_VOICE` — all default on), the vision lane (`HEARTH_TELEGRAM_VISION_LANE`, `HEARTH_TELEGRAM_VISION_MODE`, `HEARTH_TELEGRAM_VISION_PROVIDER`, `HEARTH_TELEGRAM_VISION_MODEL`, `HEARTH_TELEGRAM_VISION_MAX_ITEMS` default 8 for automatic batches, `HEARTH_TELEGRAM_VISION_LIST_CAP` default 16 for previews, `HEARTH_TELEGRAM_VISION_PER_MINUTE` default 0 (unlimited), `HEARTH_TELEGRAM_VISION_DAILY_CAP` default 0 (unlimited), `HEARTH_TELEGRAM_VISION_MAX_BYTES`, `HEARTH_TELEGRAM_VISION_RESIDENCY`), plus the Jev variables in `docs/jev.md`. Keep the database under the mounted `./data` directory so update and callback idempotency survives container restarts.
